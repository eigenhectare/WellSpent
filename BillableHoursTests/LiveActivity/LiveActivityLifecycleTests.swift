...
final class LiveActivityLifecycleTests: XCTestCase {
    ...
    func testFailedAuthoritativeStopLeavesHandoffQueuedUntilSuccessfulRetry() throws {
        ...
        XCTAssertEqual(
            try BillableHoursStopHandoff.pendingRequests(suiteName: fixture.handoffSuiteName),
            [request]
        )
...
private final class LiveActivityModelFixture {
    ...
    let handoffSuiteName = "BillableHoursLiveActivityTests.\(UUID().uuidString)"
    ...
    init() throws {
        ...
        try? BillableHoursStopHandoff.clear(suiteName: handoffSuiteName)
    }
    ...
    func makeModel() -> BillableHoursAppModel {
        BillableHoursAppModel(
