...
struct LiveActivityProjection: Equatable, Sendable {
    ...
    var runningState: WellSpentActivityAttributes.ContentState {
        WellSpentActivityAttributes.ContentState(
            phase: .running,
...
final class ActivityKitLiveActivityLifecycle: LiveActivityLifecycle {
    ...
    func switchActivity(
        ...
        for activity in activities where activity.attributes.activityID == previous.sessionID {
            let finalState = WellSpentActivityAttributes.ContentState(
                phase: .stopped,
    ...
    private var activities: [Activity<WellSpentActivityAttributes>] {
